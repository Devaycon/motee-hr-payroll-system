...
const FILE = path.join(DATA_DIR, "recruitment.json");

interface RecruitmentFile {
...
async function read(): Promise<RecruitmentFile | null> {
  try {
    const raw = await fs.readFile(FILE, "utf8");
    const parsed = JSON.parse(raw);
    if (parsed && typeof parsed === "object" && parsed.byCountry) {
      return parsed as RecruitmentFile;
    }
    return null;
  } catch {
    return null;
