...
import { hydrate, type RecruitmentBucket } from "./recruitment-slice";

const STORAGE_KEY = "motee:recruitment";
...
export function initRecruitmentPersistence(): void {
  ...
  let last: Record<string, RecruitmentBucket> | null = null;
  let putTimer: ReturnType<typeof setTimeout> | null = null;

  store.subscribe(() => {
    const byCountry = store.getState().recruitment.byCountry;
